...
\midrule
1 & Read raw ultrasonic distances on each node and convert them into far/near occupancy states using explicit thresholds (far sensor placed $\sim$40 m upstream; each sensor uses a 100 cm detection window around itself). \\
...
Far threshold & 100 cm & Detection window around the far sensor (placed $\sim$40 m upstream). \\
Near threshold & 100 cm & Detection window around the near sensor (at the stop line). \\
...
\begin{tabularx}{\textwidth}{>{\bfseries}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.16\textwidth} >{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.20\textwidth}}
\toprule
Sensor zone & Zone length & Max speed implied by 200 ms & Assumed max speed & Required loop for that speed \\
\midrule
Far threshold (100 cm) & 1.00 m & $\le$ 5.0 m/s $\approx$ 18 km/h & 30 km/h (8.33 m/s) & $\le$ 1.00/8.33 s $\approx$ 120 ms \\
Near threshold (100 cm) & 1.00 m & $\le$ 5.0 m/s $\approx$ 18 km/h & 15 km/h (4.17 m/s) & $\le$ 1.00/4.17 s $\approx$ 240 ms \\
\bottomrule
\end{tabularx}
\caption{Sampling-interval justification: current 200 ms loop assumes near-stop traffic; higher approach speeds require a faster loop or a wider detection zone.}
\end{table}
...
Far ultrasonic threshold & 100 cm & ProjectConfig.h & Detection window around the far sensor (placed $\sim$40 m upstream). \\
Near ultrasonic threshold & 100 cm & ProjectConfig.h & Detection window around the near sensor (at the stop line). \\
